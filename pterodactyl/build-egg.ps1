...
$egg = [ordered]@{
    ...
    variables     = @(
        ...
        (New-Var "Loader type" "forge for Minecraft 1.17 and newer, forge-legacy for 1.16 and older, or fabric. This picks how the server is launched, so getting it wrong stops the server from starting." "LOADER_TYPE" "forge" "required|string|in:forge,forge-legacy,fabric"),
        (New-Var "Loader version" "Exact mod loader version. For Forge this is the Forge build, for example 47.4.1. For Fabric this is the Fabric loader version, for example 0.15.11." "LOADER_VERSION" "47.4.1" "required|string|max:32"),
